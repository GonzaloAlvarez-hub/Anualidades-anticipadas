...
#A continuacion abordare las anualidades anticipadas con dos enfoques, desde el valor final y desde el valor actual
...
ANLDnperiodosConVF=function(VF,A,r){
  Tpers=log((VF*r)/(A*(1+r))+1)/log(1+r)
  Tpers=round(Tpers,0)
  return(Tpers)
}
...
ANLDtasaPeriodoConVF=function(VF,A,Tpers){
  ...
  r=tasaAnualidadAnticipadaVF(VF,A,Tpers,umbral=10*(10^-18))
  return(r)
}
...
ANLDnperiodosConVA=function(VA,A,r){
  Tpers= -log(1-(VA*r)/(A*(1+r)))/log(1+r)
  Tpers=round(Tpers,0)
  return(Tpers)
}
...
ANLDtasaPeriodoConVA=function(VA,A,Tpers){
  source("https://www.dropbox.com/s/s1nv2dypd2z0f3e/funcionesRMateFin.R?raw=1")
  r=tasaAnualidadAnticipadaVA(VA,A,Tpers,umbral=10*(10^-18))
  return(r)
}
